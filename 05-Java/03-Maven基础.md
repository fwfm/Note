...
### 依赖的排除  
1. 有时候为了确保程序正确，可以将有可能重复的间接依赖排除。(自已pom.xml中的版本和间接依赖的版本可能不一致)    
...
例如：Default (or Build) 生命周期中的`compile`阶段就是插件`maven-compiler-plugin`的一个功能。
...
### 创建父工程
创建父工程和一般的java工程操作一致，唯一需要注意的是:**打包方式要设置为pom**。  
